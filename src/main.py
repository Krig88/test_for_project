...
if __name__ == "__main__":
    print("Demo")
    fg = FieldGenerator(Coordinates(5, 4))  # (first - x, second -y)
    field = fg.get_field()
    env = Environment(field, 0)
    env.register_area(Player, (Coordinates(1, 2), Coordinates(3, 4)))
    # for _ in range(1):
    #     fg = FieldGenerator(Coordinates(5, 10)) #(first - x, second -y)
    #     field = fg.get_field()
    #     view = FullFieldView(field)
    #     player, cat, dog = Player(distance_of_view=2), Cat(), Dog()
