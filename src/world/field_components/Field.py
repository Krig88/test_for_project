...
class Field:
    def __init__(self, cells: list[list[Cell]], field_size: tuple[int, int],
                 actors: dict[Actor:tuple[int, int]] = None) -> None:
        self.actors = {}
        self.cells = cells
    ...
    def out_of_bounds_check(self, cur_position: tuple[int, int], direction: tuple[int, int]):
        ...
        if step_to_cords[0] < 0 or step_to_cords[0] >= self.field_size[0] or step_to_cords[1] < 0 or step_to_cords[1] >= \
                self.field_size[1]:
            raise Exception
